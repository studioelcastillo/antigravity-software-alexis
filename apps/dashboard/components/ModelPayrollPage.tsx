...
const ModelPayrollPage: React.FC = () => {
  ...
  const loadLiquidation = async () => {
    ...
    try {
      ...
      const response = await LiquidationService.getModelsLiquidation({
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">
            Liquidacion de Modelos
          </h1>
          <p className="text-sm text-slate-500 mt-1 font-medium">
            Consolida ingresos, deducciones y pagos por periodo.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            ...
            <tbody className="divide-y divide-slate-50 text-xs">
              {loading ? (
                ...
                dataset.map((row) => (
                  <tr key={`${row.user_id}-${row.std_id}`} className="hover:bg-amber-50/10">
